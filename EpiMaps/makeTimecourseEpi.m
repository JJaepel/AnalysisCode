...
for condition = 1:metadata.StimParams.uniqStims
    t = ((0:(numberOfFrames-1))/metadata.Imaging.rate)-analysisParams.pre;
    y = nanmedian(analysis.(analysisParams.field).roi.stimResponseTrace(:,:,condition,logical(analysis.maskA19(:))),4); % Takes median response of the pixel values. Reduces noise relative to mean
    yMean     = squeeze(mean(y,2));
    yStdError = std(y,[],2)/sqrt(size(y,2));
    shadedErrorBar(t,yMean,yStdError,'lineProps',{'-','LineWidth',3,'Color',LUT(condition,:)},'patchSaturation',0.4); hold on;
end
xlabel('Time (s)');
ylabel('Response amplitude A19');
axis square;
set(gca,'Box','off');
set(gcf, 'color', 'w');
saveas(gcf, fullfile(analysisParams.saveDirectory, 'Timecourse_A19.png'))
close gcf

figure
for condition = 1:metadata.StimParams.uniqStims
    t = ((0:(numberOfFrames-1))/metadata.Imaging.rate)-analysisParams.pre;
    y = nanmedian(analysis.(analysisParams.field).roi.stimResponseTrace(:,:,condition,logical(analysis.maskV1(:))),4); % Takes median response of the pixel values. Reduces noise relative to mean
    yMean     = squeeze(mean(y,2));
    yStdError = std(y,[],2)/sqrt(size(y,2));
    shadedErrorBar(t,yMean,yStdError,'lineProps',{'-','LineWidth',3,'Color',LUT(condition,:)},'patchSaturation',0.4); hold on;
end
xlabel('Time (s)');
ylabel('Response amplitude V1');
axis square;
set(gca,'Box','off');
set(gcf, 'color', 'w');
...
saveas(gcf, fullfile(analysisParams.saveDirectory, 'Timecourse_V1.png'))
close gcf