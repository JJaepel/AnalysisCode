...
showEpiRespAvg(analysis, metadata,trialAveragedMaps, orientationAveragedMaps, directionAveragedMaps, sfAveragedMaps, field, mapType,saveDirectory)
...
for i = 1:nStims
    ...
    imagesc(polarMap(dirMapSf, [0 clipValue(2)]));
    axis image; axis off;
    caxis([0 360]);
    ...
    imagesc(polarMap(oriMapSf, clipValue));
    axis image; axis off;
    caxis([0 180]);
    cb=colorbar; colormap(cb, hsv);
    title([spatFreq_cell{i}, ' cpd'])
