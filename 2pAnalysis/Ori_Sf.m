...
direction = linspace(0,360, numStims+1);
...
for i = 1:length(data.roi)
    Response_sf_prefOri = zeros(metadata.StimParams.numSf,metadata.StimParams.numTrials-1,size(analysis.(field).roi(i).stimResponseTrace,3));
    for sf = 1:metadata.StimParams.numSf
        temp = analysis.(field).roi(i).stimResponseTrace(1+(sf-1)*ori:sf*ori, 1:metadata.StimParams.numTrials-1, :);
    ...
    semResponse_sf_prefOri = (std(avgResponse_sf_prefOri,[],2)/sqrt(size(medResponse_sf_prefOri,2)))';
    ...
    %plot(cellfun(@str2double,spatFreq_cell),medResponse_sf_prefOri)
end

%compute DSI and OSI at prefered SF
for i=1:length(data.roi)
    prefSfResponse = analysis.(field).roi(i).stimResponseTrace((analysis.(field).roi(i).prefSfStim-1)*lastOrientation+1:analysis.(field).roi(i).prefSfStim*lastOrientation, 1:metadata.StimParams.numTrials-1, :);
    ...
    [analysis.(field).roi(i).OSI, ~] = computeOSI(lastOrientation/2,prefSfResponse_ori);
...
for types = 1:3
    ...
    subplot(2,3,3)
    PlotPrefOnTemplate(analysis, data, metadata,2, field,template, rois_sf)
...
function PlotPrefOnTemplate(analysis, data, metadata,type, field,template,rois)
    ...
    for i = 1:length(rois)
        ...
        try
            ...
            elseif type == 2
                plot(xpos,ypos,'ok','MarkerSize',12,'MarkerFaceColor',coc_sf(analysis.(field).roi(i).prefSfStim,:));
            elseif type == 3
