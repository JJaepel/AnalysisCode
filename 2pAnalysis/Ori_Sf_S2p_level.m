function Ori_Sf_S2p_level(animal, expt_id, sp2id, name, reloadData, plotROIs)
...
if metadata.StimParams.numOrientations > 2
    direction = linspace(0,360, numStims+1);
else
    direction = linspace(0,180, numStims+1);
end
...
for i = 1:length(data.roi)
    ...
    if metadata.StimParams.numOrientations > 2
        analysis.(field).roi(i).preferredDirection = analysis.(field).roi(i).preferredDirection/2;
    end
    ...
    if metadata.StimParams.numOrientations > 2
        ...
        ind = round(analysis.(field).roi(i).preferredDirection / 180); 
        analysis.(field).roi(i).prefOriStimInd = ind+1;
        if isnan(analysis.(field).roi(i).prefOriStimInd) || analysis.(field).roi(i).prefOriStimInd > 2
            analysis.(field).roi(i).prefOriStimInd = 1;
        end
    end
end
    ...
    for i = 1:length(rois)
        ...
        try
            ...
            elseif type == 2
                ...
                plot(xpos,ypos,'ok','MarkerSize',sizeMarker,'MarkerFaceColor',coc_sf(analysis.(field).roi(l).prefSfStim,:));
            elseif type == 3
