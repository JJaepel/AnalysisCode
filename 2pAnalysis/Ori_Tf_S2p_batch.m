...
[~, xls_txt, xls_all]=xlsread('Z:\Juliane\Organization\Animals\2pExpByStimulus.xlsx', 'driftingGrating_ori_tf');
...
for i = ind
    ...
    if exp_info.vol{i} == 1
        Ori_Tf_S2p_level(char(exp_info.animal{i}), char(exp_info.exp_id{i}), char(exp_info.sp2_id{i}), char(exp_info.name{i}), reloadData, plotROIs);
    else
        Ori_Tf_S2p(char(exp_info.animal{i}), char(exp_info.exp_id{i}), char(exp_info.sp2_id{i}), char(exp_info.name{i}), reloadData, plotROIs);
    end
end
