...
analysisParams.plotROIs = 1;   %should you plot traces for resp ROIs?
...
analysisParams.makeROIs = 0;

analysisParams.zThresh = 5;
analysisParams.fraction = 0.7;
analysisParams.predictor = 1;
analysisParams.shufflenum = 100;
analysisParams.field = 'dff';
analysisParams.windowStart = 0;
analysisParams.windowStop = 1.75;
analysisParams.pre = 1;
analysisParams.post = 0.5;

%% list all experiments 
if analysisParams.server
    [~, xls_txt, xls_all]=xlsread('Z:\Juliane\Organization\Animals\2pExpByStimulusAxon.xlsx', 'Retinotopy');
else
    [~, xls_txt, xls_all]=xlsread('F:\Organization\Animals\2pExpByStimulusAxon.xlsx', 'Retinotopy');
end
