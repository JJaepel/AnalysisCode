...
[~, xls_txt, xls_all]=xlsread('F:\Organization\Animals\2pExpByStimulus.xlsx', 'Retinotopy');
...
for i = 2:size(xls_txt,1)
    ...
    if flag{1} == 0
        ...
        exp_info.special{k} = xls_all(i,specCol);
...
plotROIsResp = 0;   %should you plot traces for all ROIs?
reloadData = 0; %should you reload from suite2p and do baselining?
select = 1;
...
for i = ind
    ...
    %try
        if exp_info.vol{i} == 1
            Patches_S2p_level(char(exp_info.animal{i}), char(exp_info.exp_id{i}), char(exp_info.sp2_id{i}), char(exp_info.name{i}), reloadData, plotROIsResp, string(exp_info.special{i}));
        else
            Patches_S2p(char(exp_info.animal{i}), char(exp_info.exp_id{i}), char(exp_info.sp2_id{i}), char(exp_info.name{i}), reloadData, plotROIsResp, string(exp_info.special{i}));
        end
%     catch
%         disp('Error in the analysis')
%     end
end