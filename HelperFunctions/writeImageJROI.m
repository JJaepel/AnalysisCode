...
function roi_name = writeImageJROI(matlabpathcoors, method, ch,z,t, folder)
    ...
    slice = z; % Slice = 1 works for also for single slice images. IJ will just ignore the slice value.
...
function [roi_name] = getroiname(slice, cntry, cntrx)
    ...
    yval = num2str(ceil(cntry),'%04d'); 
    xval = num2str(ceil(cntrx),'%04d'); 
