function multiAxonExperiment(animal, folder, server, level)
...
saveBaseDir = [basedir 'ImageAnalysis\' animal filesep];
saveDirectory = [saveBaseDir folder filesep];
...
if extractROIs
    Suite2pAxonExtractorFct(animal, folder, server, level)
...
for i = 1:length(idxs)
    exp = char(idxs{i});
    Sp2Directory = ([basedir 'Spike2Data\' animal filesep exp filesep]);
    files=dir(strcat(Sp2Directory, '\*.py'));
    files={files.name};
    for f = 1:length(files)
        if isempty(strfind(char(files{f}), 'serialTriggerDaqOut'))
            StimType= strrep(char(files{f}), '.py', '');
        end
    end
    ...
    try
        ...
        switch StimType
            case 'driftingGrating'
                if reloadData
                    Ori_Grating_S2p_Axon(animal, exp, exp, folder, reloadData, makeROIs, plotROIs, server) 
                end
            case 'driftingGrating_ori_sf'
                if reloadData
                    Ori_Sf_S2p_Axon(animal, exp, exp, folder, reloadData, makeROIs, plotROIs, server)
                end
...
%                     end
        ...
        expt_id{ind} = exp;
        datapath = [saveBaseDir exp filesep];
        filename = [datapath filesep '*ana.mat'];
        files = dir(filename);
        master{ind} = load(fullfile(datapath, files(1).name), 'data', 'metadata', 'analysis');
        disp(['loaded Data from exp ' exp])
        ind = ind + 1;
    catch
...
%% combine Data into one structur
...
for i = 1:length(combined.analysis.roi)
    combined.analysis.roi(i).peaks = master{1}.analysis.dff.roi(i).peaks(1:end-1,1:min(combined.metadata.numTrials));
    combined.analysis.roi(i).zscore = master{1}.analysis.dff.roi(i).zscore(1:end-1,1:min(combined.metadata.numTrials));
    ...
    combined.analysis.roi(i).stimResponseTrace = master{1}.analysis.dff.roi(i).stimResponseTrace(1:end-1,1:min(combined.metadata.numTrials),:);
    ...
    for exp = 2:length(expt_id)
        combined.analysis.roi(i).stimResponseTrace =  cat(1,combined.analysis.roi(i).stimResponseTrace,master{exp}.analysis.dff.roi(i).stimResponseTrace(1:end-1,1:min(combined.metadata.numTrials),:));
        ...
        combined.analysis.roi(i).peaks =  cat(1,combined.analysis.roi(i).peaks,master{exp}.analysis.dff.roi(i).peaks(1:end-1,1:min(combined.metadata.numTrials)));
        combined.analysis.roi(i).zscore =  cat(1,combined.analysis.roi(i).zscore,master{exp}.analysis.dff.roi(i).zscore(1:end-1,1:min(combined.metadata.numTrials)));
...
for i = 1:length(combined.analysis.roi)
    medResponse_ori = zeros(combined.metadata.numDirections,min(combined.metadata.numTrials),size(combined.analysis.roi(i).stimResponseTrace,3));
    endnum = numConditions * combined.metadata.numDirections;
    %collapse all conditions for the same direction
    for direction = 1:combined.metadata.numDirections
        medResponse_ori(direction,:,:) = mean(combined.analysis.roi(i).stimResponseTrace(direction:combined.metadata.numDirections:endnum, 1:min(combined.metadata.numTrials), :));
    end
...
for i = 1:length(combined.analysis.roi)
    ResponseCondPrefOri = zeros(numConditions,min(combined.metadata.numTrials),size(combined.analysis.roi(i).stimResponseTrace,3));
    for con = 1:numConditions
        temp = combined.analysis.roi(i).stimResponseTrace(1+(con-1)*lastOrientation*2:con*lastOrientation*2, min(combined.metadata.numTrials), :);
        ResponseCondPrefOri(con, 1, :) = (temp(combined.analysis.roi(i).prefOriStimInd,:,:)+temp(combined.analysis.roi(i).prefOriStimInd+lastOrientation/2,:,:)/2);
    end
...
for i=1:length(combined.analysis.roi)
    prefConResponse = combined.analysis.roi(i).stimResponseTrace((combined.analysis.roi(i).prefConStim-1)*lastOrientation*2+1:combined.analysis.roi(i).prefConStim*lastOrientation*2, 1:min(combined.metadata.numTrials), :);
...
for i = 1:length(combined.analysis.roi)
    if combined.analysis.roi(i).isResponseSignificant == 1
        PlotHeatMapResponse(combined, i)
        saveas(gcf, fullfile(ROIRespsaveDirectory, ['ROI_Nr_' num2str(i) '_Heat_.png']))
        close gcf
    end
end
...
if plotROIs
    for i = 1:length(combined.analysis.roi)
        if combined.analysis.roi(i).isResponseSignificant == 1
            ...
            saveas(gcf, fullfile(ROIRespsaveDirectory, ['ROI_Nr_' num2str(i) '_ResponseMap_.png']))
            close gcf
        else
            PlotTrialStimResponse(combined, i)
...
function PlotHeatMapResponse(combined, i)
    avgTrace = combined.analysis.roi(i).avgResponseTrace;
    conditionArray = reshape(avgTrace, [combined.metadata.numDirections size(avgTrace,2) combined.metadata.numConditions]);
    figure
    for con = 1:combined.metadata.numConditions
        subplot(combined.metadata.numConditions, 1, con)
        imagesc(conditionArray(:,:,con))
        ylabel(['sf = ' num2str(combined.metadata.spatialFreq(con)) 'cpd'])
    end
    xlabel('frame')
end