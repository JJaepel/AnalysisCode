function Suite2pAxonTifCombiner(analysisParams)
...
dirName         = [baseDirectory analysisParams.animal '\' analysisParams.name '\suite2p\'];
saveDir         = [baseDirectory analysisParams.animal '\' analysisParams.name '\suite2p\combined\reg_tif\'];
if ~exist(saveDir, 'dir')
    mkdir(saveDir);
end

% look for folders and how many tifs are inside
folders = dir([dirName 'plane*']);
for planes = 1:length(folders)
    tifFiles{planes} = dir([dirName folders(planes).name '\reg_tif\' '*.tif']);
end
...
if length(folders) == 4
    for tifNr = 1:size(tifFiles{1},1)
        tifStack = [];
        for planes = 1:length(folders)
            tifStack = cat(3, tifStack,read_Tiffs([dirName folders(planes).name '\reg_tif\' tifFiles{planes}(tifNr).name], 1, 50));
        end
