...
startSize = 10
numSizes = 4
stimSizes = numpy.linspace(startSize, numSizes* startSize, numSizes)
centerPoint = [0, 0]
temporalFreq = 3
spatialFreq = 0.1
contrast = 1
...
gratingStim1 = visual.GratingStim(win=myWin, tex=textureType, units='deg', mask='circle', contrast=1,
                                  pos=centerPoint, size=[stimSizes[1],stimSizes[1]], sf=spatialFreq, autoLog=False)
gratingStim2 = visual.GratingStim(win=myWin, tex=textureType, units='deg', mask='circle', opacity=1, contrast=1,
                                  pos=centerPoint, size=[stimSizes[0],stimSizes[0]], sf=spatialFreq, autoLog=False)
...
for trial in range(0, numTrials):
    stimAnnulusOrder = range(0, len(stimSizes)+1)
    ...
    for stimAnnulusNumber in stimAnnulusOrder:
        trigger.preStim(stimAnnulusNumber + 1)
        if stimAnnulusNumber > len(stimSizes)-1:
            gratingStim1.setContrast(0)
            gratingStim2.setContrast(0)
            print "\tStim", int(stimAnnulusNumber + 1), 'blank'
        else:
            if stimAnnulusNumber == 0:
                gratingStim1.size = [stimSizes[0], stimSizes[0]]
                gratingStim2.size = [0.0001, 0.0001]
                print "\tStim", int(stimAnnulusNumber + 1), 'Stimulussize', stimSizes[0], 'deg'
            else:
                sizeNumberGrat1 = int(stimAnnulusNumber)
                sizeNumberGrat2 = int(stimAnnulusNumber-1)
                print "\tStim", int(stimAnnulusNumber + 1), 'Stimulussize', stimSizes[sizeNumberGrat1], 'deg'
                gratingStim1.size = [stimSizes[sizeNumberGrat1], stimSizes[sizeNumberGrat1]]
                gratingStim2.size = [stimSizes[sizeNumberGrat2], stimSizes[sizeNumberGrat2]]
