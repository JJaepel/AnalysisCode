...
sys.path.append("C:\\Users\\fitzlab1\\Documents\\Psychopy\\psychopy\\Dan\\triggers")
from os import path
print "initialized"
...
# Grating parameter
temporalFreq =(0.2, 0.8, 1.6, 3.2, 6.4) #12.8 additional, 1.6 additional
spatialFreq = 0.08
if blankpercent > 0:
    nBlank = int(math.floor(len(orientations) * len(temporalFreq) * blankpercent))
contrast = 1
...
print "\n", str(len(orientations) * len(temporalFreq) + doBlank), "stims will be run for", str(numTrials), "trials."
if nBlank > 1:
    print "Will run blank " + str(nBlank) + " times"
# force a wait period of at least 5 seconds before first stim
duration = initialDelay + numTrials * numOrientations * len(temporalFreq) * (isi + stimDuration) + numTrials * nBlank
...
for trial in range(0, numTrials):
    ...
    stimOrder = range(0, len(orientations) * len(temporalFreq) + doBlank)
    #stimOrder = numpy.linspace(0, len(orientations) * len(temporalFreq), len(orientations)*len(temporalFreq)+1)
    if nBlank > 1:
        blankID = len(orientations) * len(temporalFreq)
    ...
    for stimNumber in stimOrder:
        ...
        if stimNumber == len(orientations) * len(temporalFreq):
            ...
            tf = math.ceil(stimNumber / len(orientations))
            gratingStim.ori = orientations[int(ori)] - 90
            print "\tStim", stimNumber + 1, orientations[int(ori)],'deg', temporalFreq[int(tf)], 'Hz'
        clock.reset()
