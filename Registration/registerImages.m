function [diffphase, row_shift, col_shift] = registerImages(referenceImage,sourceImage,filterParameters,skipReferenceImage)
...
if(nargin >= 3)
    ...
    if(skipReferenceImage)
        refImage = real(bandpassFermiFilter_Revised(refImage,...
            lowPassCutOffWavelength,highPassCutOffWavelength,spatialResolution,0.05));
    end
    srcImage = real(bandpassFermiFilter_Revised(srcImage,...
        lowPassCutOffWavelength,highPassCutOffWavelength,spatialResolution,0.05));
end

% pad images
[refImage, ~] = padImage(refImage);
[srcImage, ~] = padImage(srcImage);
