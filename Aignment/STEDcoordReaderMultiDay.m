...
pixelperUm = 10;
animal = 'F2688';
projectFolder = 'Y:\Data_JJ_Ferret_project\JJ_FerretProject_Organized\';

%%
%Step 1: read in data from excel file
coordDataFile = [projectFolder animal filesep animal '.xlsx'];
sheets = sheetnames(coordDataFile);

%now go through each data individually
for sheet = 2%:1%length(sheets)
    %Step 1: read in data from sheet
    currSheet = sheets(sheet);
    [~, xls_txt, xls_all]=xlsread(coordDataFile, currSheet);
    ...
    tifFolder = ['Y:\Data_JJ_Ferret_project\JJ_FerretProject_Organized\' char(animal) filesep char(currSheet) '\Copies\'];
