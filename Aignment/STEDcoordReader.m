...
if max(coordInfo(i).xPos) > 0
    startPosLeft = max([coordInfo.AbsPosLeftX]);
    endPosRight = min([coordInfo.AbsPosRightX]);
else
    startPosLeft = min([coordInfo.AbsPosLeftX]);
    endPosRight = max([coordInfo.AbsPosRightX]);
end
...
for i =1:length(coordInfo)
    if coordInfo(i).AbsPosLeftX > 0
        ...
        coordInfo(i).RelPosLeftX = coordInfo(i).AbsPosLeftX-startPosLeft;
        coordInfo(i).RelPosRightX = coordInfo(i).AbsPosRightX-startPosLeft;
    end
