...
plotROIs = 1;   %should you plot traces for all ROIs?
reloadData = 1; %should you reload from suite2p and do baselining?
select = 1; %load only selected data (1, marked in column run) or all data (0)?
server = 0; %load from the server (1) or the raid (0)

% list all experiments
if server
    [~, xls_txt, xls_all]=xlsread('J:\Juliane\Organization\Animals\2pExpByStimulusAxon.xlsx', 'driftingGrating');
else
    [~, xls_txt, xls_all]=xlsread('F:\Organization\Animals\2pExpByStimulusAxon.xlsx', 'driftingGrating');
end
...
for i = ind
    ...
    %else
        Ori_Grating_S2p_Axon(char(exp_info.animal{i}), char(exp_info.exp_id{i}), char(exp_info.sp2_id{i}), char(exp_info.name{i}), reloadData, plotROIs, server);
    %end
end